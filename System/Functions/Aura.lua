...
function br.UnitBuffID(unit, spellID, filter)
	...
	if exactSearch then
		...
		if filter ~= nil and br._G.strfind(br._G.strupper(filter), "PLAYER") then return br._G.AuraUtil.FindAuraByName(
			spellName, unit, "HELPFUL|PLAYER") end
...
function br.UnitDebuffID(unit, spellID, filter)
	...
	if exactSearch then
		...
		if filter ~= nil and br._G.strfind(br._G.strupper(filter), "PLAYER") then return br._G.AuraUtil.FindAuraByName(
			spellName, unit, "HARMFUL|PLAYER") end
